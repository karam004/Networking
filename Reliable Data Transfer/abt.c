...
//: declaration of all functions to remove "implicit warning" messages from compiler
...
struct pkt packetVar;

//checksum Function 
// Based on TCP checksum as suggested in PA2 guide.
...
//This function check if  a packet is corrupt or not.It returns 1 when the packet is corrupt and 0 when its not.
...
{
	//make packet
	strncpy(packetVar.payload,message.data,20);
	A_application++;
	if(ACKWAIT == 0 && !toggle)
	{
		//send packet
		packetVar.seqnum = 0;
		packetVar.acknum = 0;
		packetVar.checksum = createCheckSum(packetVar); 
		ACKWAIT = 1;
		toggle = 1;
		A_transport++;
		tolayer3(0,packetVar);
		starttimer(0,TIMEOUT);
	}
	else if(ACKWAIT == 0 && toggle)
	{
		//send packet
		packetVar.seqnum = 1;
		packetVar.acknum = 1;
...
void A_input(packet)
  struct pkt packet;
{
		if((isCorrupt(packet) == 0) && packetVar.seqnum == packet.seqnum) // acking the last sent packet
		{
			stoptimer(0);
			ACKWAIT = 0;
		}
		else
		printf("\n SideA:Error Case in A_input with recieved packet %d and last sent packet %d and isCorrupt = %d",packet.seqnum,packetVar.seqnum,isCorrupt(packet));
}
  ...
  struct pkt packet;
{
	B_transport++;
	struct pkt packetB;
	if((isCorrupt(packet) == 0) && packet.seqnum == expectedSeqNum)
	{
		//got the expected packet. Send to application and sent ACK for it.
		tolayer5(1,packet.payload);
		B_application++;
		strcpy(packet.payload,"CORRECTPACKET");
		packetB.acknum = expectedSeqNum;
		packetB.seqnum = expectedSeqNum;
		packetB.checksum = createCheckSum(packetB); 
		tolayer3(1,packetB);
		if(expectedSeqNum)
			expectedSeqNum= 0;
		else
	...
	else if((isCorrupt(packet) == 0 && packet.seqnum != expectedSeqNum))
	{
		//Got unexpected packet , Send ACK with seqnuence number same as current packet to mark as duplicate.
		strcpy(packet.payload,"WRONGPACKET");
		packetB.acknum = packet.acknum;
		packetB.seqnum = packet.seqnum;
		packetB.checksum = createCheckSum(packetB); 
		tolayer3(1,packetB);
	}
	else
		printf("\n Side B: Not counting packet %d, it returns %d for corrupt and expectedSeqnum here was %d",packet.seqnum,isCorrupt(packet),expectedSeqNum);
